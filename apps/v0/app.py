...
SHOWBAGS_PATH = Path("../../data/showbags/showbags.csv")
...
def query_showbags(query, chat_history=None):
    ...
    try:
        ...
        retriever = VectorIndexRetriever(
            index=index,
            similarity_top_k=3,  # Retrieve top 3 showbags for display
        )
        ...
        if hasattr(response, 'source_nodes') and response.source_nodes:
            for i, node in enumerate(response.source_nodes[:3], 1):
                formatted_response += f"### Showbag {i}:\n"
                formatted_response += format_showbag_for_display(node)
                formatted_response += "\n---\n\n"
            
            # Add the LLM response as a summary/recommendation
